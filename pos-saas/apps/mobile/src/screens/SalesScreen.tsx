...
export function SalesScreen() {
  ...
  const [cart, setCart] = useState<Array<{ product: ProductRecord; quantity: number }>>([]);
  ...
  const addToCart = (product: ProductRecord) => {
    ...
    if (product.stock < currentQty + 1) {
      setErrorMsg(`Stock insuficiente para ${product.name} (Disponible: ${product.stock}).`);
      setTimeout(() => setErrorMsg(null), 3000);
      return;
    }
  ...
  async function handleConfirmCloseRegister() {
    ...
    try {
      ...
      setErrorMsg(err instanceof Error ? err.message : "Error al cerrar la caja.");
      setCheckingSession(false);
    }
  }
      ...
      <SafeAreaView style={styles.safeArea}>
        <ScrollView contentContainerStyle={styles.centerContainer} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.formCard}>
            ...
            <Pressable style={styles.primaryButton} onPress={handleOpenRegister}>
      ...
      <SafeAreaView style={styles.safeArea}>
        <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
          <View style={styles.headerRow}>
            <Pressable onPress={() => setSalesView("pos")} style={styles.backLink}>
          ...
          <View style={styles.summaryCard}>
            <View style={styles.summaryRow}>
              ...
              <Text style={styles.summaryValue}>$ {activeSession?.opening_amount.toFixed(2)}</Text>
            </View>
            <View style={styles.summaryRow}>
              <Text style={styles.summaryLabel}>Ventas en Efectivo:</Text>
              <Text style={styles.summaryValue}>
                $ {(expectedCash - (activeSession?.opening_amount ?? 0)).toFixed(2)}
              </Text>
            </View>
            <View style={[styles.summaryRow, styles.summaryTotalRow]}>
              <Text style={styles.summaryTotalLabel}>Saldo Teórico Esperado:</Text>
              <Text style={styles.summaryTotalValue}>$ {expectedCash.toFixed(2)}</Text>
            </View>
          </View>
          ...
          <View style={styles.formCard}>
            ...
            <Pressable style={[styles.dangerButton, { marginTop: 10 }]} onPress={handleConfirmCloseRegister}>
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.posHeader}>
        ...
        <Pressable style={styles.closeRegisterLink} onPress={handleInitCloseRegister}>
      ...
      <View style={styles.posContainer}>
        ...
        {searchQuery.trim() ? (
          <View style={styles.searchResultsContainer}>
            <Text style={styles.searchTitle}>Catálogo ({searchResults.length})</Text>
            <FlatList
              data={searchResults}
              keyExtractor={(item) => item.id}
              keyboardShouldPersistTaps="handled"
              renderItem={({ item }) => (
                <Pressable style={styles.searchResultRow} onPress={() => {
                  addToCart(item);
                  setSearchQuery("");
                  setSearchResults([]);
                }}>
                  <View style={styles.searchResultInfo}>
                    ...
                    <Text style={styles.searchResultMeta}>
                      Stock: {item.stock} {item.unit} · Barcode: {item.barcode ?? "Sin código"}
                    </Text>
                  </View>
                    ...
                    <View style={styles.cartRow}>
                      <View style={styles.cartItemInfo}>
                        ...
                        <Text style={styles.cartItemPrice}>
                          $ {item.product.sale_price.toFixed(2)} x {item.product.unit}
                        </Text>
                      </View>
        ...
        <Pressable style={styles.modalOverlay} onPress={() => setCheckoutVisible(false)}>
          <View style={styles.modalContainer} onStartShouldSetResponder={() => true}>
            ...
            <View style={styles.checkoutTotalCard}>
              ...
              <Text style={styles.checkoutTotalValue}>$ {cartTotal.toFixed(2)}</Text>
            </View>
            ...
            {paymentMethod === "cash" ? (
              <View style={styles.cashForm}>
                ...
                {change !== null ? (
                  <View style={styles.changeCard}>
                    <Text style={styles.changeLabel}>Vuelto para el cliente:</Text>
              ...
              <View style={styles.transferForm}>
                <Text style={styles.transferNotice}>
                  Solicitá al cliente la transferencia e ingresá los fondos al saldo bancario.
                </Text>
              </View>
            )}
            ...
            <View style={styles.checkoutActions}>
              ...
              <Pressable
                style={[styles.confirmCheckoutBtn, savingSale && styles.buttonDisabled]}
                onPress={() => {
...
const getStyles = (colors: ThemeColors, isDark: boolean) => StyleSheet.create({
  ...
  dangerButton: {
    ...
    borderWidth: 1,
    borderColor: isDark ? "rgba(255, 180, 180, 0.22)" : "rgba(211, 47, 47, 0.18)",
    alignItems: "center",
  },
  dangerButtonText: {
    color: isDark ? "#FFB4B4" : "#D32F2F",
  ...
  errorBanner: {
    backgroundColor: isDark ? "rgba(255, 180, 180, 0.15)" : "rgba(211, 47, 47, 0.08)",
    borderWidth: 1,
    borderColor: isDark ? "rgba(255, 180, 180, 0.35)" : "rgba(211, 47, 47, 0.18)",
    padding: 10,
    borderRadius: 10,
  },
  errorBannerText: {
  ...
  deleteBtn: {
    ...
    backgroundColor: isDark ? "rgba(255, 180, 180, 0.08)" : "rgba(211, 47, 47, 0.08)",
    borderWidth: 1,
    borderColor: isDark ? "rgba(255, 180, 180, 0.15)" : "rgba(211, 47, 47, 0.18)",
    alignItems: "center",
    justifyContent: "center",
  },
  deleteBtnText: {
  ...
  successBanner: {
    backgroundColor: isDark ? "rgba(122, 230, 179, 0.15)" : "rgba(1, 203, 99, 0.08)",
    borderWidth: 1,
    borderColor: isDark ? "rgba(122, 230, 179, 0.35)" : "rgba(1, 203, 99, 0.18)",
    padding: 10,
    borderRadius: 10,
    marginBottom: 8,
  },
  ...
  paymentOptionActive: {
    ...
    backgroundColor: isDark ? "rgba(138, 199, 255, 0.16)" : "rgba(4, 151, 191, 0.15)",
  },
  paymentOptionText: {
    color: colors.textMuted,
    fontSize: 14,
