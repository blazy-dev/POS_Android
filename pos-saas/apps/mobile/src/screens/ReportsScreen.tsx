  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.tabSelector}>
        ...
        >
          <Text style={[styles.tabButtonText, activeTab === "cash" && styles.tabButtonTextActive]}>
            Historial de Caja
          </Text>
        </Pressable>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer} keyboardShouldPersistTaps="handled">
        ...
        {activeTab === "sales" ? (
          <View style={styles.tabContent}>
            ...
            <View style={styles.heroMetricsCard}>
              ...
              <Text style={styles.metricHeroValue}>$ {summary.sales_total.toFixed(2)}</Text>
            </View>

            <View style={styles.metricsGrid}>
              <View style={styles.metricMiniCard}>
                <Text style={styles.metricMiniLabel}>Efectivo</Text>
                <Text style={[styles.metricMiniValue, { color: colors.success }]}>
              ...
              <View style={styles.metricMiniCard}>
                ...
                <Text style={[styles.metricMiniValue, { color: colors.primary }]}>
            ...
            <View style={styles.metricMiniCardFull}>
              <View style={styles.rowBetween}>
                <Text style={styles.metricMiniLabel}>Transacciones completadas:</Text>
                <Text style={styles.metricTxCount}>{summary.transaction_count} ticket(s)</Text>
              </View>
            </View>

            {/* Listado de más vendidos */}
            <View style={styles.sectionCard}>
              <Text style={styles.sectionTitle}>Productos Más Vendidos (Top 5)</Text>
                    ...
                    return (
                      <View key={item.id} style={styles.topProductRow}>
                        ...
                        <View style={styles.progressBarBg}>
                          <View style={[styles.progressBarFill, { width: widthPercent as any }]} />
                        </View>
        ...
        {activeTab === "cash" ? (
          <View style={styles.tabContent}>
            ...
            {sessions.length === 0 ? (
              <Text style={styles.emptyText}>No hay sesiones de caja registradas.</Text>
            ) : (
              <View style={styles.timeline}>
                {sessions.map((item) => {
                  const isOpen = item.status === "open";
                  return (
                    <View key={item.id} style={[styles.timelineCard, isOpen && styles.timelineCardOpen]}>
                      <View style={styles.timelineHeader}>
                        <View>
                          <Text style={styles.timelineTitle}>Sesión: {item.id.substring(0, 8)}...</Text>
                          <Text style={styles.timelineUser}>Operador: {item.opened_by_name ?? "Desconocido"}</Text>
                        </View>
                        <View style={[styles.statusBadge, isOpen ? styles.statusBadgeOpen : styles.statusBadgeClosed]}>
                          <Text style={[styles.statusBadgeText, isOpen ? styles.statusBadgeTextOpen : styles.statusBadgeTextClosed]}>
                      ...
                      <View style={styles.timelineDetails}>
                        ...
                        <View style={styles.timelineDetailRow}>
                          <Text style={styles.timelineLabel}>Monto Inicial:</Text>
                          <Text style={styles.timelineValue}>$ {item.opening_amount.toFixed(2)}</Text>
                        </View>
                        ...
                        {!isOpen && item.closed_at ? (
                          <>
                            ...
                            <View style={styles.timelineDetailRow}>
                              <Text style={styles.timelineLabel}>Arqueo de Cierre:</Text>
                      ...
                      <View style={styles.timelineActions}>
                        ...
                        >
                          <Text style={styles.expandSalesBtnText}>
                            {expandedSessions[item.id] ? "▲ Ocultar desglose de ventas" : "▼ Ver desglose de ventas"}
                          </Text>
                        </Pressable>
                      </View>
                      ...
                      {expandedSessions[item.id] ? (
                        <View style={styles.sessionSalesContainer}>
                          <Text style={styles.salesHeaderTitle}>Desglose de Ventas (Tickets)</Text>
                          {!sessionSales[item.id] ? (
                            <ActivityIndicator size="small" color={colors.primary} style={{ marginVertical: 12 }} />
                          ) : sessionSales[item.id].length === 0 ? (
                            <Text style={styles.noSalesText}>No se registraron ventas en esta sesión.</Text>
                          ) : (
                              ...
                              {sessionSales[item.id].map((sale) => (
                                <View key={sale.id} style={styles.saleItemCard}>
                                  <View style={styles.saleItemHeader}>
                                    ...
                                    <Text style={styles.saleItemTotal}>$ {sale.total.toFixed(2)}</Text>
                                  </View>

                                  <View style={styles.saleItemDetails}>
                                    {sale.items.map((prod, idx) => (
                                      <Text key={idx} style={styles.saleDetailProductRow}>
                                        • {prod.product_name} x {prod.quantity} {prod.product_unit} · ${prod.unit_price.toFixed(2)} = ${prod.subtotal.toFixed(2)}
                                      </Text>
                                    ))}
                                  </View>
                                </View>
        ...
        {activeTab === "stock" ? (
          <View style={styles.tabContent}>
            <Text style={styles.sectionTitle}>Alertas de Existencias Críticas</Text>
            <Text style={styles.sectionSubtitle}>
            ...
            {lowStock.length === 0 ? (
              <View style={styles.stockSuccessCard}>
                <Text style={styles.stockSuccessText}>✓ Todo el inventario tiene stock saludable.</Text>
              </View>
            ) : (
              <View style={styles.stockList}>
                {lowStock.map((item) => {
                  ...
                  return (
                    <View key={item.id} style={[styles.stockCard, isZero && styles.stockCardZero]}>
                      <View style={styles.stockInfo}>
                        <Text style={styles.stockName}>{item.name}</Text>
                        <Text style={styles.stockPrice}>Precio Venta: $ {item.sale_price.toFixed(2)}</Text>
                      </View>
                      <View style={styles.stockValueContainer}>
                        <Text style={[styles.stockValue, isZero ? styles.stockValueZero : styles.stockValueLow]}>
                          {item.stock}
                        </Text>
...
const getStyles = (colors: ThemeColors, isDark: boolean) => StyleSheet.create({
  ...
  errorCard: {
    margin: spacing.xl,
    padding: spacing.md,
    backgroundColor: isDark ? "rgba(255, 180, 180, 0.1)" : "rgba(211, 47, 47, 0.08)",
    borderRadius: radius.md,
    borderWidth: 1,
    borderColor: isDark ? "rgba(255, 180, 180, 0.2)" : "rgba(211, 47, 47, 0.18)",
  },
  errorText: {
  ...
  heroMetricsCard: {
    backgroundColor: isDark ? "rgba(138, 199, 255, 0.04)" : colors.surfaceCard,
  ...
  timelineCardOpen: {
    borderColor: isDark ? "rgba(122, 230, 179, 0.22)" : "rgba(1, 203, 99, 0.22)",
    backgroundColor: isDark ? "rgba(122, 230, 179, 0.02)" : "rgba(1, 203, 99, 0.02)",
  },
  timelineHeader: {
    flexDirection: "row",
    justifyContent: "space-between",
  ...
  statusBadgeOpen: {
    backgroundColor: isDark ? "rgba(122, 230, 179, 0.12)" : "rgba(1, 203, 99, 0.12)",
  },
  statusBadgeClosed: {
    backgroundColor: isDark ? "rgba(255, 255, 255, 0.06)" : colors.surfaceSoft,
  },
  statusBadgeText: {
    fontSize: 10,
    fontWeight: "800",
  ...
  stockCardZero: {
    borderColor: isDark ? "rgba(255, 180, 180, 0.22)" : "rgba(211, 47, 47, 0.22)",
    backgroundColor: isDark ? "rgba(255, 180, 180, 0.02)" : "rgba(211, 47, 47, 0.02)",
  },
  stockInfo: {
    gap: 4,
    flex: 1,
